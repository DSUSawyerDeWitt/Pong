...
class Game:
    ...
    def display_score(self):
        #player
        player_surf = self.font.render(str(self.score['player']), True, COLORS['bg detail']) #picture
        player_rect = player_surf.get_frect(center = (WINDOW_WIDTH / 2 + 100, WINDOW_HEIGHT / 2 )) #location
        self.display_surface.blit(player_surf, player_rect) #draw with the picture and location

        #oppenent
        player_surf = self.font.render(str(self.score['opponent']), True, COLORS['bg detail']) #picture
        player_rect = player_surf.get_frect(center = (WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT / 2 )) #location
        self.display_surface.blit(player_surf, player_rect) #draw with the picture and location
